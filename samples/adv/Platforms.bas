...
Option Platform wx h  
' if files exists, include lib/core/[platform].h and stc[platform].cpp for console / stw[platform].cpp for gui application 
' h: ext optional, for Using platform.XXX.YYY  ---> #include <[platform]/XXX/YYY[.ext]>
' if settings exists, compiles using 
'	[platform]_GCC : gcc compiler root path, include bin, include, lib, ... paths
'	[platform]_CXX : gcc/g++ filename
'	[platform]_AR  : ar filename
'	[platform]_CPP_FLAGS [platform]_LD_FLAGS [platform]_INCLUDE [platform]_LIBRARY

Option Console Off ' On(default): stc[platform].cpp / Off: stw[platform].cpp
